...
public class HealthBar : MonoBehaviour
{
    ...
    public void Initialise(ThinkingGenerable p)
    {
        originalHP = currentHP = p.hitPoints;
        transformToFollow = p.transform;

        bar.GetComponent<Image>().color = (p.faction == Generable.Faction.Player) ? red : blue;
            
        wholeWidget.transform.localPosition = new Vector3(0f,
														(p.gType == Generable.GenerableType.Unit) ? 3f : 6f,
														(p.gType == Generable.GenerableType.Unit) ? 0f : -2f); //set the vertical position based on the type of Placeable
